...
function setupEventListeners() {
  exportBtn.addEventListener("click", async () => {
    ...
    exportBtn.textContent = "Guardando...";
...
function normalizarRegistros(registros) {
  return (
    registros
      // validación primero
      .filter((r) => {
        if (!r.nombre && !r.bebe) return false;
      ...
      // luego normalización
      .map((r) => ({
        nombre: (r.nombre || r.bebe || "").trim(),
        documento: String(r.documento).trim(),
        fecha: r.fecha || new Date().toISOString().slice(0, 10),
        dia: r.dia || "",
        asistencia: Boolean(r.asistencia),
        programa: r.programa || "CIDI",
      }))
  );
}
...
// GIDs de cada hoja del Google Sheet maestro
const SHEET_GIDS = {
  Lunes: "0",
  Martes: "1223082696",
  Miercoles: "113785064",
...
function sheetCsvUrl(dia) {
  // Usa el proxy del servidor — resuelve CORS
...
async function loadMasterDataFromServer() {
  updateSyncStatus("loading", "Sincronizando listado maestro...");
  try {
    ...
    saveToLocalStorage();
    exportBtn.disabled = false;
    updateSyncStatus("ok", `Listado actualizado — ${masterData.length} bebés`);
    console.log(`✅ Google Sheet cargado — ${masterData.length} bebés únicos`);
  } catch (err) {
    updateSyncStatus("error", "Sin conexión — usando datos guardados");
    console.warn("No se pudo cargar el Google Sheet:", err.message);
  }
}
...
function exportToExcel() {
  ...
  // Ordenar columnas y traducir headers
  ...
  // Guardar en historico del servidor
  ...
  fetch("/api/historico/guardar", {
    ...
    .then((r) => r.json())
    .then((r) => {
      if (r.ok) console.log("Guardado en historico:", r.archivo);
    })
    .catch((e) => console.warn("No se pudo guardar en historico:", e.message));

  console.log(`Exportado exitosamente el dia: ${dayToExport}`);

  // Eliminar Extras y NoCidi, limpiar campos del resto
...
async function guardarEnNube(registros) {
  if (enviando) {
    alert(" Ya se está enviando información...");
    return;
  }
  ...
  try {
    // 1. Normalizar
    const datosNormalizados = normalizarRegistros(registros);

    // 2. Eliminar duplicados
    const datosFinales = eliminarDuplicados(datosNormalizados);
    ...
    // 3. Info de duplicados (pro)
    const eliminados = datosNormalizados.length - datosFinales.length;

    if (eliminados > 0) {
      console.log(` ${eliminados} duplicados eliminados`);
    }

    // 4. Enviar datos correctos
    const res = await fetch("/api/historico/guardar", {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
        "x-api-key": "TU_API_KEY",
      },
      body: JSON.stringify({ datos: datosFinales }),
    });
    ...
    alert(
      ` Guardado exitoso (${data.total} registros)` +
        (eliminados > 0 ? `\n ${eliminados} duplicados ignorados` : ""),
    );
  } catch (error) {
    console.error(error);
