...
const UnifiedModel = (() => {
  ...
  function _normalizeRow(rawRow) {
    ...
    // Normalizar Edad a rangos estándar (acepta números y strings)
    if (out.Edad) {
      const n = parseInt(String(out.Edad).replace(/\D/g, ""), 10);
      if (!isNaN(n)) {
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  setupBabySearch();
  restoreSession();
  cargarHistoricoServidor(); // carga automática si hay servidor
});

/** Intenta cargar archivos desde el servidor (opcional, falla silenciosamente) */
async function cargarHistoricoServidor() {
  try {
    ...
    const yaEnSesion = new Set(loadedFiles.map((f) => f.name));
    const nuevos = archivos.filter((a) => !yaEnSesion.has(a.nombre));
    if (!nuevos.length) return;
    showLoading(true);
    const blobs = await Promise.all(
      nuevos.map(async (a) => {
        const buf = await (await fetch(a.url)).arrayBuffer();
        return new File([buf], a.nombre, {
          type: "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
        });
      }),
    );
    await processFiles(blobs);
    showToast(
      `${blobs.length} archivo(s) del histórico cargados automáticamente`,
    );
  } catch {
    // No hay servidor — modo standalone (normal en local)
  }
}
...
window.clearSession = function () {
  ...
  clearCharts();
  dashGrid.innerHTML = "";
  [dashGrid, kpiStrip, filtersRow, sheetRow, babySection].forEach((el) =>
...
function getResumenBebes(data) {
  ...
  for (const r of data) {
    ...
    if (!map.has(bKey)) {
      ...
      prev._diasTotal++;
      if (esSi(val(r, "asistencia"))) {
        prev._diasPresente++;
        prev.Asistencia = "Sí"; // el resumen es "Sí" si asistió al menos un día
      }
      if (esSi(val(r, "reporte"))) {
        prev._reportes++;
        prev.Reporte = "Sí";
      }
      // Enriquecer campos de perfil vacíos con datos de días posteriores
...
function showUI() {
  ...
  [kpiStrip, filtersRow, babySection, dashGrid].forEach((el) =>
  ...
  if (!document.getElementById("btnConsolidar")) {
    ...
    `;
    btn.innerHTML = `📊 Exportar tabla consolidada`;
    btn.onmouseover = () => {
    ...
    // Insertar junto al botón "Limpiar todo" en el área de carga
    const fileLoaded = document.getElementById("fileLoaded");
    if (fileLoaded) fileLoaded.appendChild(btn);
  }
}
...
function updateKPIs(resumen, todosLosRegistros) {
  // resumen = 1 entrada por bebé (incluye Extras y NoCidi)
  const total = resumen.length;
  // Presente = asistió al menos un día (_diasPresente > 0)
  const presentes = resumen.filter((r) => (r._diasPresente || 0) > 0).length;
  const ausentes = total - presentes;
  // Con reporte = tiene al menos un reporte de ausencia (_reportes > 0)
  const reportes = resumen.filter((r) => (r._reportes || 0) > 0).length;
  // Extras y NoCidi: sub-conteos informativos (ya incluidos en total/presentes/ausentes)
  const nExtras = resumen.filter((r) => esSi(val(r, "visitante"))).length;
  const nNoCidi = resumen.filter((r) => esSi(val(r, "nocidi"))).length;

  // Etiqueta de período
  const diasUnicos = [
    ...new Set(todosLosRegistros.map((r) => val(r, "dia")).filter(Boolean)),
  ];
  const diaLabel =
    diasUnicos.length === 1
      ? diasUnicos[0]
      : diasUnicos.length > 1
        ? `${diasUnicos.length} días`
        : `${loadedFiles.length} archivo(s)`;
  ...
  document.getElementById("kpiPresentePct").textContent =
    `${total ? Math.round((presentes / total) * 100) : 0}% del total`;
  document.getElementById("kpiAusentes").textContent = ausentes;
  document.getElementById("kpiAusentePct").textContent =
    `${total ? Math.round((ausentes / total) * 100) : 0}% del total`;
  document.getElementById("kpiReportes").textContent = reportes;
  const elExtras = document.getElementById("kpiExtras");
  const elNoCidi = document.getElementById("kpiNocidi");
  if (elExtras) elExtras.textContent = nExtras;
  if (elNoCidi) elNoCidi.textContent = nNoCidi;
}
...
function buildDashboardForFilter(filter) {
  ...
  switch (filter) {
    ...
    default: {
      ...
      if (hasRep) buildChartTasaReporte(resumen);
      if (hasUbic) buildChartUbicacion(resumen);
      if (hasDia) buildChartAsistenciaPorDia(registros);
      if (hasInst) buildChartInstitucion(resumen);
      if (hasProg) buildChartPrograma(resumen);
    ...
    <div style="padding:20px 28px;display:flex;align-items:center;gap:28px;flex-wrap:wrap;">
      ...
      <div style="display:flex;gap:24px;flex-wrap:wrap;align-items:center;">
        <div style="text-align:center;">
          ...
          <div style="font-family:'JetBrains Mono',monospace;font-size:9px;letter-spacing:1.5px;color:var(--muted);text-transform:uppercase;margin-top:4px">bebés</div>
        </div>
        <div style="text-align:center;">
          <div style="font-size:40px;font-weight:800;color:var(--text-mid);line-height:1">${total}</div>
          <div style="font-family:'JetBrains Mono',monospace;font-size:9px;letter-spacing:1.5px;color:var(--muted);text-transform:uppercase;margin-top:4px">total</div>
        </div>
        <div style="text-align:center;">
          <div style="font-size:40px;font-weight:800;color:${color};line-height:1">${pct}%</div>
          <div style="font-family:'JetBrains Mono',monospace;font-size:9px;letter-spacing:1.5px;color:var(--muted);text-transform:uppercase;margin-top:4px">del total</div>
        </div>
      </div>
...
function buildChartDistribucion(data) {
  const presentes = data.filter(
    (r) => (r._diasPresente || 0) > 0 || esSi(val(r, "asistencia")),
  ).length;
...
/** Dona: Tasa de reporte entre los ausentes */
function buildChartTasaReporte(data) {
  const ausentes = data.filter(
    (r) => !((r._diasPresente || 0) > 0) && !esSi(val(r, "asistencia")),
  );
  const reportaron = ausentes.filter(
    (r) => (r._reportes || 0) > 0 || esSi(val(r, "reporte")),
  ).length;
  ...
  card.querySelector(".chart-wrap").innerHTML = `
    ...
    <div style="position:absolute;top:50%;left:50%;transform:translate(-50%,-50%);text-align:center;pointer-events:none">
      <div style="font-size:26px;font-weight:800;color:var(--naranja)">${ausentes.length}</div>
      <div style="font-family:'JetBrains Mono',monospace;font-size:9px;letter-spacing:1.5px;color:var(--muted)">ausentes</div>
    </div>`;
  dashGrid.appendChild(card);
  charts["chart-reporte"] = new Chart(
    document.getElementById("chart-reporte").getContext("2d"),
    {
      ...
      data: {
        ...
        datasets: [
          {
            data: [reportaron, ausentes.length - reportaron],
      ...
      options: { ...chartOpts(true), cutout: "68%" },
    },
  );
}

/** Dona: Distribución por ubicación */
...
function buildChartInstitucion(data) {
  ...
  data.forEach((r) => {
    ...
    if (!c) return;
    const asistio = (r._diasPresente || 0) > 0 || esSi(val(r, "asistencia"));
    asistio ? counts[c].presentes++ : counts[c].ausentes++;
  });
...
function buildChartPrograma(data) {
  ...
  function canonProg(raw) {
    const n = norm(raw);
    if (n === "hoteleria" || n === "hotelería") return "Hotelería";
...
function buildChartEdad(data) {
  ...
  data.forEach((row) => {
    ...
    // Primero: intentar con el rango normalizado ("6-15" o "16-30")
    if (raw === "6-15") bucket = "6 – 15 meses";
    else if (raw === "16-30") bucket = "16 – 30 meses";
    else {
      // Segundo: intentar con número
    ...
    const asistio =
      (row._diasPresente || 0) > 0 || esSi(val(row, "asistencia"));
    asistio ? BUCKETS[bucket].presentes++ : BUCKETS[bucket].ausentes++;
  });
...
function buildChartCausasDirecto(data) {
  ...
  const counts = {};
  data.forEach((r) => {
    const canon = canonizarSituacion(String(r[cSit] || "").trim());
    if (!canon) return;
    counts[canon] = (counts[canon] || 0) + 1;
  });
  const entries = SIT_CANON.filter((c) => counts[c.key] > 0).sort(
  ...
  const total = entries.reduce((s, c) => s + counts[c.key], 0);
...
function buildETLSection(registros, resumen) {
  ...
  buildCollapsible("etl-ranking", "🏆 Ranking — Bebés con más ausencias", () =>
    buildETLRanking(registros),
  );
  buildCollapsible("etl-comparacion", "📆 Comparación por período", () =>
    buildETLComparacion(registros),
  );
 ...
 */
function buildETLRanking(data) {
  const body = document.getElementById("collapsible-body-etl-ranking");
  if (!body) return;
...
function buildETLComparacion(data) {
  const body = document.getElementById("collapsible-body-etl-comparacion");
  if (!body) return;
